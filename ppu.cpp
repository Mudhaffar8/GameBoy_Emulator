...
{
    int tile_map_y = (screen_y + scroll_y) & 0xFF;

    // SCX mod 8 => pixels to discard from left
    // 8 - (SCX mod 8) => pixels to discard from right tile 
    int tile_offset_x = scroll_x % GBTile::SIZE_PIXELS;
    ...
    for (int tile_x = 0; tile_x < GBResolution::TILES_PER_ROW_VISIBLE_MAX; ++tile_x)
    {
        ...
        int tile_map_x = (screen_x + scroll_x) & 0xFF;
        std::pair<uint8_t, uint8_t> tile_row = fetch_tile_row(tile_map_x, tile_map_y);
        
        if (debug_mode) 
            std::cout << "Tile Map Coordinates: (" << tile_map_x << ", " << tile_map_y << ")\n";
        ...
        if (debug_mode)
        {
            std::cout << "Screen Coordinates: (" << last_tile_screen_x << ", " << screen_y << ")\n";
            std::cout << '\n';
        }
    }
}
...
void Ppu::decode_tile_row(uint8_t hi_byte, uint8_t lo_byte, int tile_start_x, int screen_y)
{
    ...
    for (int i = 7; i >= 0; --i)
    {
        int pixel_screen_x = tile_start_x + i;
...
{ 
    ...
    int tile_x = (bg_map_x / GBTile::SIZE_PIXELS) & 0x1F;
    int tile_y = (bg_map_y / GBTile::SIZE_PIXELS) & 0x1F;
