    ...
    )
    {
        ...
        for (int i = start; i < end; ++i)
        {
            ...
            for (json::iterator it = data.begin(); it != data.end(); ++it)
            {
                ...
                cpu.get_A() = static_cast<uint8_t>((*it)["initial"]["a"]);
                cpu.get_F()  = static_cast<uint8_t>((*it)["initial"]["f"]);
                cpu.get_bc().high = static_cast<uint8_t>((*it)["initial"]["b"]);
                cpu.get_bc().low = static_cast<uint8_t>((*it)["initial"]["c"]);
                cpu.get_de().high = static_cast<uint8_t>((*it)["initial"]["d"]);
                cpu.get_de().low = static_cast<uint8_t>((*it)["initial"]["e"]);
                cpu.get_hl().high = static_cast<uint8_t>((*it)["initial"]["h"]);
                cpu.get_hl().low = static_cast<uint8_t>((*it)["initial"]["l"]);
                cpu.get_pc() = static_cast<uint16_t>((*it)["initial"]["pc"]);
                cpu.get_sp() = static_cast<uint16_t>((*it)["initial"]["sp"]);
                cpu.get_halted_state() = false;
                ...
                check_val(cpu.get_A(), static_cast<uint8_t>((*it)["final"]["a"]), "A");
                check_val(cpu.get_F(), static_cast<uint8_t>((*it)["final"]["f"]), "F");
                check_val(cpu.get_bc().high, static_cast<uint8_t>((*it)["final"]["b"]), "B");
                check_val(cpu.get_bc().low, static_cast<uint8_t>((*it)["final"]["c"]), "C");
                check_val(cpu.get_de().high, static_cast<uint8_t>((*it)["final"]["d"]), "D");
                check_val(cpu.get_de().low, static_cast<uint8_t>((*it)["final"]["e"]), "E");
                check_val(cpu.get_hl().high, static_cast<uint8_t>((*it)["final"]["h"]), "H");
                check_val(cpu.get_hl().low, static_cast<uint8_t>((*it)["final"]["l"]), "L");
                check_val(cpu.get_pc(), static_cast<uint16_t>((*it)["final"]["pc"]), "PC");
                check_val(cpu.get_sp(), static_cast<uint16_t>((*it)["final"]["sp"]), "SP");
            ...
            std::fill(mem.get_mem_ref().begin(), mem.get_mem_ref().end(), 0x00);
        }
    }
}